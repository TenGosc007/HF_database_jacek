  ...
  pool.getConnection((err, con)=>{
    ...
    con.query(sql, (err, res, cols)=>{
      ...
      for(var i=0; i<res.length; i++){
        table +='<tr><td>'+ (i+1) +'</td><td>'+ res[i].first_name +'</td><td>'+ res[i].position +'</td></tr>';
      }
      table ='<table border="1"><tr><th>Nr.</th><th>Name</th><th>Position</th></tr>'+ table +'</table>';
...
let sql ='SELECT first_name, position FROM users';
